...
TEST_CASE("Automatic Casting"){
  struct A: public revisited::Visitable<A> { int value = 0; };
  struct B: public revisited::DerivedVisitable<B, A> { B(){ value = 1; } };
  struct C: public revisited::DerivedVisitable<C, B> { C(){ value = 2; } };
  ...
  SUBCASE("pass by reference"){
    ...
    REQUIRE_THROWS(f(B(),C()).get<A>());  // currently a problem with visitable types: cannot be captured by value
  }
