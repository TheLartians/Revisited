...
class SpecificAnyFunction<R, Args...> : public SpecificAnyFunctionBase {
private:
  std::function<R(Args...)> callback;

  template <size_t... Idx>
  Any callWithArgumentIndices(const AnyArguments &args,
...
public:
  SpecificAnyFunction(std::function<R(Args...)> _callback)
      : callback(_callback) {}
...
template <class R>
class SpecificAnyFunction<R, const AnyArguments &>
    : public SpecificAnyFunctionBase {
private:
  std::function<R(const AnyArguments &)> callback;

public:
  SpecificAnyFunction(std::function<R(const AnyArguments &)> _callback)
      : callback(_callback) {}
...
private:
  ...
  template <class R, typename... Args>
  void _set(const std::function<R(Args...)> &f) {
...
public:
  ...
  template <typename F, typename = typename std::enable_if<
                            !std::is_convertible<F, AnyFunction>::value>::type>
    ...
    AnyArguments arguments{{[&]() {
      ...
      if constexpr (std::is_base_of<Any, ArgType>::value) {
        return AnyReference(args);
                               ...
                               typename AnyVisitable<ArgType>::type::Type,
                               ArgType>::value) {
        return AnyReference(
            std::reference_wrapper<typename std::remove_reference<Args>::type>(
                args));
      } else {
        return AnyReference(args);
      }
