...
class VisitorPrototype
    : public virtual VisitorBasePrototype<SingleBase, Single>,
      public Single<Args>... {
private:
...
using Visitor = VisitorPrototype<SingleVisitorBase, SingleVisitor, Args...>;
...
template <class T>
class SingleRecursiveVisitor : public SingleRecursiveVisitorBase {
public:
  /**
...
template <typename... Args>
using RecursiveVisitor = VisitorPrototype<SingleRecursiveVisitorBase,
                                          SingleRecursiveVisitor, Args...>;
...
static void visit(V *visitable, TypeList<T, Rest...>, VisitorBase &visitor) {
  if (auto *v = visitor.asVisitorFor<T>()) {
    if constexpr (std::is_base_of<IndirectVisitableBase,
 ...
 */
template <class V, class T, typename... Rest>
static bool visit(V *visitable, TypeList<T, Rest...>,
...
public:
  ...
  template <typename... Args>
  DerivedVisitable(Args &&... args) : B(std::forward<Args>(args)...) {}
  using B::accept;
...
public:
  using InheritanceList =
      revisited::InheritanceList<>::Merge<typename Bases::InheritanceList...>;
...
class alignas(Bases...) VirtualVisitable : public virtual Bases... {
public:
  using InheritanceList =
      revisited::InheritanceList<>::Merge<typename Bases::InheritanceList...>;
...
class DataVisitableWithBasesAndConversionsDefinition<
    T, TypeList<Bases...>, TypeList<Conversions...>, CastType> {
private:
  using ConstTypes =
      typename TypeList<const T &, const Bases &..., Conversions...>::
          template Merge<typename TypeList<T, Bases...>::template Filter<
              std::is_copy_constructible>>;

public:
  using type = DataVisitablePrototype<
      T, typename TypeList<T &, Bases &...>::template Merge<ConstTypes>,
      ConstTypes, CastType>;
};
...
using DataVisitableWithBasesAndConversions =
    typename DataVisitableWithBasesAndConversionsDefinition<
        T, Bases, Conversions, CastType>::type;

/**
 * Same as DataVisitableWithBasesAndConversions, without conversions.
 */
template <class T, typename... Bases>
using DataVisitableWithBases =
