...
namespace revisited {
  ...
  template <typename... Types> struct TypeList {
    ...
    template <template <class> typename F> using Filter =
        typename typelist::Filter<TypeList, F>::type;
    template <template <class> typename T> using Transform = TypeList<typename T<Types>::type...>;
    template <class F, template <class> typename T> static constexpr auto transform(F &&f) {
      return f(T<Types>()...);
    }
  };
  ...
  namespace typelist {
    ...
    public:
      using type = typename std::conditional<
          F<First>::value, typename TypeList<First>::template Merge<RestType>, RestType>::type;
    };
