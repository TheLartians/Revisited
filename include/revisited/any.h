...
template <class T>
constexpr static bool NotDerivedFromAny =
...
public:
  ...
  decltype(auto) setWithBases(Args &&... args) {
    return set<T, DataVisitableWithBases<T, Bases...>>(
  ...
  static Any withBases(Args &&... args) {
    Any a;
...
template <class T> struct AnyVisitable {
  using type =
      typename std::conditional<std::is_base_of<VisitableBase, T>::value, T,
...
#define REVISITED_DEFINE_SCALAR_TYPE(Type, Conversions)                        \
  template <> struct revisited::AnyVisitable<Type> {                           \
    ...
    using ConstTypes = typename revisited::TypeList<                           \
        const Type &>::template Merge<Conversions>;                            \
...
template <class T>
struct revisited::AnyVisitable<std::reference_wrapper<const T>> {
  using type = revisited::DataVisitablePrototype<
...
template <class T> struct revisited::AnyVisitable<std::shared_ptr<T>> {
  using type = revisited::DataVisitablePrototype<
      ...
      typename TypeList<const std::shared_ptr<T> &, std::shared_ptr<T>>::
          template Merge<typename AnyVisitable<T>::type::ConstTypes>,
      T>;
};
...
template <typename... Args> struct GetAnyVisitableTypes {
  using Types = revisited::TypeList<>::Merge<
      typename revisited::AnyVisitable<Args>::type::Types...>;
  using ConstTypes = revisited::TypeList<>::Merge<
      typename revisited::AnyVisitable<Args>::type::ConstTypes...>;
};
