...
namespace revisited {
  ...
  public:
     ...
     * casting the object. The default is `revisited::AnyVisitable<T>::type` which can be specialized
     * for usertypes.
     */
    template <
      class T,
    ...
    TypeIndex type()const{
      if (!data) { return revisited::getTypeIndex<void>(); }
...
#define LARS_ANY_NUMERIC_TYPES ::revisited::TypeList<char, unsigned char, short int, unsigned short int, int, unsigned int, long int, unsigned long int, long long int, unsigned long long int, float, double, long double>
#endif
...
namespace revisited {
  ...
  template <typename ... Args> struct GetAnyVisitableTypes {
    using Types = revisited::TypeList<>::Merge<typename revisited::AnyVisitable<Args>::type::Types ...>;
    using ConstTypes = revisited::TypeList<>::Merge<typename revisited::AnyVisitable<Args>::type::ConstTypes ...>;
  };

} 

#define LARS_ANY_DECLARE_BASES(TYPE,...) template <> struct revisited::AnyVisitable<TYPE> { \
  using Types = revisited::TypeList<TYPE&>::Merge<typename GetAnyVisitableTypes<__VA_ARGS__>::Types>; \
  using ConstTypes = revisited::TypeList<const TYPE &>::Merge<typename GetAnyVisitableTypes<__VA_ARGS__>::ConstTypes>; \
  using type = revisited::DataVisitablePrototype<TYPE, Types, ConstTypes>; \
}
